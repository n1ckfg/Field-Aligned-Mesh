void keyPressed() {
    ...
    if (key == 'o') M.opposite();
    if (key == 'p') M.previous();
    if (key == 'q') COLLAPSER.AnalyseCollapseAnalyse(); //{COLLAPSER.collapse(FMESH.AM.c); FMESH.AM.c = FMESH.AM.s(FMESH.AM.c);}
    if (key == 'r') M.right();
    if (key == 's') M.swing();
    ...
    if (key == CODED) {
        ...
        if (keyCode == LEFT) {
            pressed = "LEFT";
            TRACER.maxTraceCount = max(TRACER.maxTraceCount-1, 0);
        };
        if (keyCode == RIGHT) {
            pressed = "RIGHT";
            TRACER.maxTraceCount += 1;
        };
        if (keyCode == ALT) {
            pressed = "ALT";
        };
...
void mouseDragged() {
    ...
    if (keyPressed && key == '.') {
        rb += 20. * (mouseX - pmouseX) / width;
    }
    if (keyPressed && key == 'h') {
        columnHeight -= 100. * (mouseY - pmouseY) / width;
    }
