...
class COTSMap
{
    ...
    float scaling(float x, float y) {
        return pow(mu, x)*pow(mv, y);
    ...
    pt ImageNotExponentialOf(pt P) {
        return spiralNotExponentialPt(spiralNotExponentialPt(A, F, mu, au, P.x), F, mv, av, P.y);
    }
    pt Transform(pt P, float u, float v) {
        return spiralPt(spiralPt(P, F, mu, au, u), F, mv, av, v);
    }
