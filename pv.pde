...
pt L(pt A, float t, pt B) {
    return P(A.x+t*(B.x-A.x), A.y+t*(B.y-A.y));
}
pt BiL(pt A, pt B, pt C, pt D, float u, float v) {
    return L(L(A, u, B), v, L(C, u, D));
}
...
vec W(float u, vec U, float v, vec V, float w, vec W) {
    return W(W(S(u, U), S(v, V)), S(w, W));
}                                   // uU+vV ( Linear combination)
...
float m(pt A, pt B, pt C) {
    return (B.x-A.x)*(C.y-A.y) - (B.y-A.y)*(C.x-A.x);
}
float a(pt P, pt A, pt B, pt C) {
    return m(P, B, C)/m(A, B, C);
}
float b(pt P, pt A, pt B, pt C) {
    return m(A, P, C)/m(A, B, C);
}
float c(pt P, pt A, pt B, pt C) {
    return m(A, B, P)/m(A, B, C);
}
