 class pts // class for manipulaitng and displaying pointclouds or polyloops in 3D 
 {
     ...
     vec[] V = new vec[maxnv]; // vector table
     ...
     int SETppToIDofVertexWithClosestScreenProjectionTo(pt M) // sets pp to the index of the vertex that projects closest to the mouse 
     {
         pp = 0;
         for (int i = 1; i<nv; i++)
             if (d(M, ToScreen(G[i]))<= d(M, ToScreen(G[pp]))) pp = i;
         return pp;
     }
     ...
     pts drawColumns(float r, float h) {
         for (int v = 0; v<nv; v++) pillar(G[v], h, r);
         return this;
     }
    ...
    void loadVecs(String fn) {
        ...
        for (int k = 0; k<nv; k++) {
            int i = k + s;
            String[] SS = split(ss[i], ",");
            V[k] = new vec(float(SS[0]), float(SS[1]), float(SS[2]));
        }
    }
