...
public pt pick(int mX, int mY) { // returns point on visible surface at pixel (mX,My)
    ...
    float[] viewport = {
        0, 0, p3d.width, p3d.height
    };
    float[] normalized = new float[4];
    normalized[0] = ((mX - viewport[0]) / viewport[2]) * 2.0*f - 1.0*f;
    normalized[1] = ((height - mY - viewport[1]) / viewport[3]) * 2.0*f - 1.0*f;
    normalized[2] = depthValue * 2.0*f - 1.0*f;
    normalized[3] = 1.0*f;
